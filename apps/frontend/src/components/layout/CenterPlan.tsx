...
export function CenterPlan({ store, progress }: { store: SyllabusStore; progress: AgentProgress }) {
  if (store.loading) return <p className="px-6 py-8 text-sm text-fg-muted">Loading syllabus…</p>;
  if (!store.syllabus) return <EmptyState />;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-4 md:px-6 md:py-6">
      ...
      <p className="mt-1 line-clamp-2 text-sm text-fg-muted">{store.syllabus.requirements}</p>
...
function EmptyState() {
  return (
    <div className="flex h-full min-h-[50vh] flex-col items-center justify-center px-6 text-center">
      <div className="mb-3 h-10 w-10 rounded-2xl bg-accent-soft text-accent grid place-items-center text-lg">✎</div>
      <h2 className="text-base font-semibold">Start a new syllabus</h2>
      <p className="mt-1 max-w-sm text-sm text-fg-muted">
        Describe what you want to teach in the right pane — we&apos;ll research, outline,
        and draft chapter by chapter, live.
      </p>
    </div>
  );
}
