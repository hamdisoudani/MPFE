...
from ..events import (
    ...
    emit_critic, emit_lesson_committed, emit_activity_committed, emit_error,
)
from ..prompts import (
    WRITER_PERSONA, WRITER_TASK_TEMPLATE, WRITER_RETRY_TEMPLATE,
    CRITIC_PERSONA, CRITIC_TASK_TEMPLATE,
    ACTIVITY_WRITER_PERSONA, ACTIVITY_WRITER_TASK_TEMPLATE,
    ACTIVITY_CRITIC_PERSONA, ACTIVITY_CRITIC_TASK_TEMPLATE,
)
from ..state import TodoPlan, TodoStep
from ..store_keys import ns_dep_summary, ns_draft, purge_namespace
from ..tools.db_tools import (
    exec_commit_lesson, exec_commit_activity,
    exec_set_chapter_status, exec_set_phase,
)
...
class QuizQuestion(BaseModel):
    prompt: str = Field(description="The question the student reads.")
    options: list[str] = Field(
        min_length=2, max_length=6,
        description="Answer options. Include plausible distractors.",
    )
    ...
    explanation: str = Field(
        description="Why the correct option is right; what the distractors get wrong.",
    )
    difficulty: Optional[str] = Field(
        default=None,
        description="'easy' | 'medium' | 'hard' — helps teachers pace.",
    )
...
class ActivityOut(BaseModel):
    ...
    instructions: str = Field(
        description="1-2 paragraphs the teacher reads aloud or the student sees."
    )
    questions: list[QuizQuestion] = Field(min_length=3, max_length=12)
    summary: str = Field(description="1-2 sentence summary for downstream steps.")
...
async def write_node(state: dict, *, store: BaseStore) -> dict:
    ...
    if step.kind == "activity":
        user = ACTIVITY_WRITER_TASK_TEMPLATE.format(
            step_id=step.id,
            ...
            dep_block=dep_block,
            search_summary=(state.get("search_summary") or "")[:4000],
            retry_block=retry_block,
        )
        try:
            llm = writer_llm().with_structured_output(ActivityOut, method="function_calling")
            out: ActivityOut = await llm.ainvoke([
                SystemMessage(ACTIVITY_WRITER_PERSONA),
                HumanMessage(user),
            ])
            payload = {
                "kind": "quiz",
            ...
            try:
                await store.aput(ns_draft(thread_id, step.id), "current", {
                    "kind": "activity",
                    "title": step.name,
                    "payload": {"kind": "quiz", "instructions": f"(writer failed: {e})", "questions": []},
                    "summary": "",
    ...
    try:
        ...
        lout: LessonOut = await llm.ainvoke([
    ...
    try:
        await store.aput(ns_draft(thread_id, step.id), "current", {
            "kind": "lesson",
            "title": lout.title,
            "content_md": lout.content_markdown,
            "summary": lout.summary,
            "attempt": step.attempts,
            "critique": None,
            "weaknesses": [],
        })
...
async def critic_node(state: dict, *, store: BaseStore) -> dict:
    ...
    if not it or not it.value:
        ...
        if step.kind == "activity":
            ...
            user = ACTIVITY_CRITIC_TASK_TEMPLATE.format(
                step_id=step.id,
                chapter_ref=step.chapter_ref,
                ...
                must_cover="\n".join(f"- {x}" for x in (step.must_cover or [])) or "(none)",
                dep_block=dep_block,
                payload_json=_json.dumps(payload, indent=2)[:14000],
            )
            sys_prompt = ACTIVITY_CRITIC_PERSONA
        else:
...
async def commit_node(state: dict, *, store: BaseStore) -> dict:
    ...
    draft = it.value
    # Position within chapter — count accepted steps in this chapter so far + 1,
    # using a per-kind sequence so lesson pos and activity pos are independent.
    pos = 1 + sum(
        1 for s in plan.steps
        if s.chapter_ref == step.chapter_ref
        and s.status == "accepted"
        and s.kind == step.kind
    )

    if step.kind == "activity":
        # Resolve lesson_id from first lesson dep (if any).
        first_lesson_id: str | None = None
        dep_lesson_ids: list[str] = []
        for dep_id in step.depends_on or []:
            dep_step = next((s for s in plan.steps if s.id == dep_id), None)
            if dep_step and dep_step.kind == "lesson" and dep_step.final_lesson_id:
                dep_lesson_ids.append(dep_step.final_lesson_id)
        first_lesson_id = dep_lesson_ids[0] if dep_lesson_ids else None

        payload = dict(draft.get("payload") or {})
        payload["lesson_refs"] = list(step.depends_on or [])
        payload["lesson_ids"] = dep_lesson_ids

        try:
            row = exec_commit_activity(
            ...
            emit_error("commit_activity", str(e))
            step.status = "failed"
            plan.steps[idx] = step
            return {"todo_plan": _to_state(plan)}
        ...
        step.final_activity_id = activity_id
        plan.steps[idx] = step
        emit_activity_committed(step.id, activity_id or "", cid, draft.get("title", step.name))
    else:
        try:
    ...
    # If this was the last step in the chapter, mark chapter as done.
